...
class BRIGHTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        optical = load_optical(opt_path, tile_size=tile_size)
        sar = load_sar(sar_path, tile_size=tile_size)
        
        # create an empty label image if path not exists
        if lbl_path.exists():
        ...
        return {
            "optical": torch.from_numpy(optical.transpose(2, 0, 1)),
            "sar":     torch.from_numpy(sar.transpose(2, 0, 1)),
