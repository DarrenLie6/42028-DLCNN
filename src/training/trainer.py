...
# trainer.py
NUM_CLASSES   = 5
IGNORE_INDEX  = None      # no ignoring - use all 5 classes
...
class Trainer:
    ...
    # public entry point
    def fit(self, start_epoch: int = 1) -> list[dict]:
        """Run the full training loop, optionally resuming from a saved epoch."""
    ...
    def plot_history(self, save_dir: str = "checkpoints") -> None:
        ...
        axes[1].set_title("Mean IoU (all 5 classes)")
        ...
        axes[2].set_title("Mean Accuracy (all 5 classes)")
        ...
        if os.path.exists(best_path):
            ...
            with torch.no_grad():
                for batch in self.val_loader:
                    pre_disaster = batch["pre_disaster"].to(self.device)
                    post_disaster = batch["post_disaster"].to(self.device)
                    targets = batch["label"].to(self.device)
                    with autocast(
                        device_type=self.device.type,
                        enabled=self.device.type == "cuda"
                    ):
                        logits = self.model(pre_disaster, post_disaster)
    ...
    def _train_epoch(self, epoch: int) -> dict:
        ...
        for batch in pbar:
            pre_disaster = batch["pre_disaster"].to(self.device)   # (B,3,H,W)
            post_disaster = batch["post_disaster"].to(self.device) # (B,3,H,W)
            ...
            with autocast(device_type=self.device.type, enabled=self.device.type == "cuda"):
                logits = self.model(pre_disaster, post_disaster)  # (B,5,H,W)
    ...
    def _val_epoch(self, epoch: int) -> dict:
        ...
        for batch in pbar:
            ...
            if torch.isnan(loss):
                ...
                print(f"  pre_disaster  min={pre_disaster.min().item():.4f} max={pre_disaster.max().item():.4f} nan={torch.isnan(pre_disaster).any()}")
                print(f"  post_disaster min={post_disaster.min().item():.4f} max={post_disaster.max().item():.4f} nan={torch.isnan(post_disaster).any()}")
                break
    ...
    def load_checkpoint(self, path: str) -> int:
        ...
        self.history = ckpt.get("history", [])  # Restore training history
        
        # Reset epochs_no_improve counter for early stopping
        self.epochs_no_improve = 0
        
        epoch = ckpt["epoch"]
        print(f"Loaded checkpoint from {path}  (epoch {epoch}, val mean_iou={self.best_mean_iou:.4f})")
        return epoch
